...
from unicon.plugins.utils import _get_current_credentials
from unicon.plugins.utils import get_current_credential
from unicon.plugins.utils import load_token_csv_file
from unicon.plugins.tests.mock.mock_device_generic import MockDeviceTcpWrapperGeneric
...
devices:
    ...
    def test_asa_learn_tokens_from_show_version(self):
        ...
        with open(self.dev.logfile) as f:
            log_contents = f.read()
        self.assertRegex(
            log_contents, r"\+\+\+ Unicon plugin asa( \(unicon\.plugins\.asa\))? \+\+\+"
        )
    ...
    def test_ios_learn_tokens_from_show_version(self):
        ...
        with open(self.dev.logfile) as f:
            log_contents = f.read()
        self.assertRegex(
            log_contents, r"\+\+\+ Unicon plugin ios( \(unicon\.plugins\.ios\))? \+\+\+"
        )
    ...
    def test_learn_tokens_conf_state(self):
        self.dev.connections.cli.command = "mock_device_cli --os generic --state config"
    ...
    def test_learn_tokens_failure_exception(self):
        self.dev.connections.cli.command = (
            'bash -c "while :; do echo -n R1# && read resp; done"'
        )
        self.dev.connections.cli.settings["LEARN_DEVICE_TOKENS"] = True
        try:
...
devices:
    ...
    def test_version_standardization(self):
        ...
        for tokens in tokens_to_test:
            standardized_tokens = discovery.standardize_token_values(
                {"version": tokens["before"]}
            )
    ...
    def test_pid_standardization(self):
        discovery = AbstractTokenDiscovery(self.mock_con)
        standardized_tokens = discovery.standardize_token_values({"pid": "QWERTY12345"})
        self.assertEqual("QWERTY12345", standardized_tokens["pid"])

        # Test that white spaces are removed
        standardized_tokens = discovery.standardize_token_values(
            {"platform": "Q WE RTY    12345"}
        )
...
devices:
    ...
    def test_pid_file_sorted(self):
        token_csv_file = os.path.join(
            Path(os.path.realpath(__file__)).parents[1], os.path.join("pid_tokens.csv")
        )
        ...
        self.assertListEqual(
            keys,
            sorted_keys,
            msg="All rows in the pid_tokens.csv file must be in ascending sorted "
...
class TestAbstractTokenDiscoveryHAConnection(unittest.TestCase):
    def test_learn_token_HA(self):
        md = MockDeviceTcpWrapperGeneric(port=0, state="asr_exec_standby, asr_login")
        md.start()
        ...
        try:
            dev.connect(init_config_commands=[], connection_timeout=60)
            dev.disconnect()
        finally:
            md.stop()
...
class TestCredentialSelection(unittest.TestCase):
    def test_consume_current_credentials_returns_matching_named_credential(self):
        shared_credential = {"username": "cisco", "password": "lab"}
        context = {
            "login_creds": ["default", "fallback"],
            "previous_credential": "default",
        }
        session = {}
        credentials = {
            "default": shared_credential,
            "fallback": {"username": "backup", "password": "backup"},
            "current_credentials": shared_credential,
        }
        ...
        self.assertEqual(current_credential, "default")

    def test_consume_current_credentials_returns_none_without_relogin_context(self):
        context = {
            "login_creds": ["default", "fallback"],
        }
        session = {}
        credentials = {
            "default": {"username": "cisco", "password": "lab"},
            "current_credentials": {"username": "cisco", "password": "lab"},
        }
        ...
        self.assertIsNone(current_credential)

    def test_get_current_credential_prefers_current_credentials_before_iterator(self):
        shared_credential = {"username": "cisco", "password": "lab"}
        context = {
            "credentials": {
                "default": shared_credential,
            ...
            "login_creds": ["default", "fallback"],
            "previous_credential": "default",
            "cred_list": ["fallback", "default"],
        }
        session = {}

        current_credential = get_current_credential(context, session)

        self.assertEqual(current_credential, "default")
        self.assertEqual(session["current_credential"], "default")
        self.assertNotIn("cred_iter", session)

    def test_get_current_credential_uses_credential_iterator_when_no_current_match(
        self,
    ):
        context = {
            "credentials": {
        ...
        self.assertEqual(current_credential, "fallback")
        self.assertEqual(session["current_credential"], "fallback")
        self.assertIn("cred_iter", session)

    def test_get_current_credential_raises_when_all_credentials_are_exhausted(self):
        context = {
            "credentials": {
                "default": {"username": "cisco", "password": "lab"},
            },
        ...
        with self.assertRaises(CredentialsExhaustedError):
            get_current_credential(context, session)
